    ...
    public class Coupled78_9_13_VanillaSourceModel
    {
        ...
        public void CreateModel(IParentAnalyzer[] analyzers, ISolver[] solvers)
        {
            ...
            {//CALCULATE vf = kP + vs
                ...
                {
                    (-(pressureTensorDivergenceAtElementGaussPoints[elem.Key][0][0] * kth) + velocities[elem.Key][0][0]) * 1000,
                    (-(pressureTensorDivergenceAtElementGaussPoints[elem.Key][0][1] * kth) + velocities[elem.Key][0][1]) * 1000,
                    (-(pressureTensorDivergenceAtElementGaussPoints[elem.Key][0][2] * kth) + velocities[elem.Key][0][2]) * 1000,
                };
            }
            model = new Model[3];
